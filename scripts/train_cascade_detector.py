...
create_samples_call = "{0} -img {1} -bgthresh -1 -bg {2} -maxxangle 0.7 -maxyangle 0.7 -maxzangle 0.5 " \
                      "-num 1000 -w {3} -h {4} -vec {5}".format(join(opencv_bin_path, create_samples_app),
                                                                args.image, negatives_list_file, w, h, vec_file_path)
...
train_cascade_call = "{0} -vec {1} -numPos 500 -bg {2} -numNeg 500 -feature_type {3} -maxFalseAlarmRate 0.1 " \
                     "-numStages 5 -w {4} -h {5} -data {6}".format(join(opencv_bin_path, train_cascade_app),
